...
public class FCFragment extends BaseFragment implements SwipRefreshLayout.OnRefreshListener {
    ...
    private void evaluate(int pager) {
        ...
        param.pageNo = pager;
        param.pageSize = 100;
        Request.startRequest(param, pager, ServiceMap.FENGCAI, mHandler, Request.RequestFeature.BLOCK);
    }
    ...
    public boolean onMsgSearchComplete(NetworkParam param) {
        if (param.key == ServiceMap.FENGCAI) {
            result = (FengcaiResult) param.result;
            multiAdapter.setData(result.data.fengcaiResult);
        }
        return false;
    }
