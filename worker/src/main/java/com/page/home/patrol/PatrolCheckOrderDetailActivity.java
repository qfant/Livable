...
import com.page.home.patrol.PatrolCheckOrderDetailResult.HistoryDetail;
...
public class PatrolCheckOrderDetailActivity extends BaseActivity {
    ...
    private void requestData() {
        PatrolCheckOrderDetailParam patrolListParam = new PatrolCheckOrderDetailParam();
        patrolListParam.recordId = patrolItem.id;
        Request.startRequest(patrolListParam, ServiceMap.getCheckOrderRecordDetail, mHandler, Request.RequestFeature.BLOCK);
    }


    public boolean onMsgSearchComplete(NetworkParam param) {
        super.onMsgSearchComplete(param);
        if (param.key == ServiceMap.getCheckOrderRecordDetail) {
            if (param.result.bstatus.code == 0) {
                PatrolCheckOrderDetailResult result = (PatrolCheckOrderDetailResult) param.result;
