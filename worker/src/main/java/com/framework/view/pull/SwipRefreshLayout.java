...
import com.page.home.patrol.PatrolPlacesActivity;
...
public class SwipRefreshLayout extends ViewGroup {
    ...
    private SwipRefreshLayoutDirection mDirection;
    ...
    public SwipRefreshLayout(Context context, AttributeSet attrs) {
        ...
        final TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.SwipRefreshLayout);
        SwipRefreshLayoutDirection direction
                = SwipRefreshLayoutDirection.getFromInt(a2.getInt(R.styleable.SwipRefreshLayout_direction, 0));
        if (direction != SwipRefreshLayoutDirection.BOTH) {
            mDirection = direction;
            mBothDirection = false;
        } else {
            mDirection = SwipRefreshLayoutDirection.TOP;
            mBothDirection = true;
        }
        a2.recycle();
...
//    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mBothDirection) {
                    if (y > mInitialMotionY) {
                        setRawDirection(SwipRefreshLayoutDirection.TOP);
                    } else if (y < mInitialMotionY) {
                        setRawDirection(SwipRefreshLayoutDirection.BOTTOM);
                    }
                    if ((mDirection == SwipRefreshLayoutDirection.BOTTOM && canChildScrollDown())
                            || (mDirection == SwipRefreshLayoutDirection.TOP && canChildScrollUp())) {
                        return false;
                    }
                }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    int targetY;
                    if (mDirection == SwipRefreshLayoutDirection.TOP) {
    ...
    public SwipRefreshLayoutDirection getDirection() {
        return mBothDirection ? SwipRefreshLayoutDirection.BOTH : mDirection;
    }

    public void setDirection(SwipRefreshLayoutDirection direction) {
        if (direction == SwipRefreshLayoutDirection.BOTH) {
            mBothDirection = true;
        } else {
            mBothDirection = false;
    ...
    // only TOP or Bottom
    private void setRawDirection(SwipRefreshLayoutDirection direction) {
